...
class ProductsModuleTest(unittest.TestCase):
    ...
    def test_add_multiple_products_without_mutating_previous_items(self):
        first=add_product([], "One", "First", "http://example.com/one"); second=add_product(first, "Two", "Description", "https://example.com/two")
    ...
    def test_description_and_link_are_required(self):
        for description in ("", "-", "   "):
            with self.subTest(description=description), self.assertRaises(ProductValidationError): add_product([], "Item", description, "https://example.com")
        for link in ("", "-", "   "):
            with self.subTest(link=link), self.assertRaises(ProductValidationError): add_product([], "Item", "Description", link)
    def test_invalid_link_is_rejected(self):
        with self.assertRaises(ProductValidationError): add_product([], "Item", "Description", "example.com")
    def test_http_and_https_links_are_accepted(self):
        self.assertEqual(add_product([], "HTTP", "Description", "http://example.com")[0]["link"], "http://example.com")
        self.assertEqual(add_product([], "HTTPS", "Description", "https://example.com")[0]["link"], "https://example.com")
